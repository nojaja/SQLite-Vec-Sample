...
async function insertVector(contents, embedding) {
  // ベクトルをデータベースに挿入
  const stmt = window.sqliteManager.db.prepare("INSERT INTO vectors(embedding, contents) VALUES (?, ?)");
  stmt
...
async function searchSimilarVectors(embedding, limit = 5) {
  const stmt = window.sqliteManager.db.prepare("SELECT *,distance FROM vectors WHERE embedding MATCH ? ORDER BY distance LIMIT 3");
